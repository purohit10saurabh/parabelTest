...
PfastreXML_train %trn_ft_file% %trn_lbl_file% %inv_prop_file% %model_dir% -S 0 -T 5 -s 0 -t 50 -b 1.0 -c 1.0 -m 10 -g 30 -a 0.8

:: testing
:: Reads test features (in %$tst_ft_file%) and model (in %model_dir%), and writes test label scores to %score_file%
PfastreXML_predict %tst_ft_file% %score_file% %model_dir%
