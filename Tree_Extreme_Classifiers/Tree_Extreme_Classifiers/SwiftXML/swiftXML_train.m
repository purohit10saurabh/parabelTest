function swiftXML_train( trn_X_Xf, item_trn_X_Xf, trn_X_Y, inv_prop, model_dir, param )

    file_trn_X_Xf = tempname;
    write_text_mat( trn_X_Xf, file_trn_X_Xf );
    
    file_item_trn_X_Xf = tempname;
    write_text_mat( item_trn_X_Xf, file_item_trn_X_Xf );
    
    file_trn_X_Y = tempname;
    write_text_mat( trn_X_Y, file_trn_X_Y );
    
    file_inv_prop = tempname;
    csvwrite( file_inv_prop, inv_prop );
    
    clear trn_X_Xf item_trn_X_Xf trn_X_Y inv_prop;
    
    cmd = sprintf('swiftXML_train %s %s %s %s %s %s', file_trn_X_Xf, file_item_trn_X_Xf, file_trn_X_Y, file_inv_prop, model_dir, get_arguments(param));
    if isunix
       cmd = ['./' cmd]; 
    end
	disp( cmd );
...
function args = get_arguments(param)
    ...
    if isfield(param,'item_log_loss_coeff')
		args = sprintf(' %s -ic %f',args,param.item_log_loss_coeff);
    end
    
    if isfield(param,'feat_imp')
        args = sprintf(' %s -f %f',args,param.feat_imp);
    end
    
    if isfield(param,'num_trn_X')
        args = sprintf(' %s -N %d',args,param.num_trn_X);
    end
end
