function score_mat = swiftXML_predict( tst_X_Xf, item_tst_X_Xf, model_dir, param )

    file_tst_X_Xf = tempname;
    write_text_mat(tst_X_Xf,file_tst_X_Xf);
    
    file_item_tst_X_Xf = tempname;
    write_text_mat(item_tst_X_Xf,file_item_tst_X_Xf);
    
    file_score_mat = tempname;
   
	clear tst_X_Xf item_tst_X_Xf;
 
    cmd = sprintf('swiftXML_predict %s %s %s %s %s', file_tst_X_Xf, file_item_tst_X_Xf, file_score_mat, model_dir, get_arguments(param));
    if isunix
        cmd = ['./' cmd];
    end
    disp( cmd );
