...
C++:
	...
	./PfastreXML_train [feature file name] [label file name] [inverse propensity file name] [model folder name] -S 0 -T 1 -s 0 -t 50 -b 1.0 -c 1.0 -m 10 -l 100 -g 30 -a 0.8 

Matlab:

	PfastreXML_train([feature matrix], [label matrix], [inverse propensity vector], [model folder name], param)
...
C++:
	...
	./PfastreXML_predict [feature file name] [score file name] [model folder name] -S 0 -T 1 -s 0 -t 50 -n 1000 

Matlab:

	[score matrix] = PfastreXML_predict([feature matrix], [model folder name], param)
...
To run PfastreXML on the EUR-Lex dataset, execute "bash sample_run.sh" (Linux) or "sample_run" (Windows, Matlab) in the PfastreXML folder.
